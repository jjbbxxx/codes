...
class NNNet(Module):
    ...
    def forward(self, x: torch.Tensor):
        # 这里会处理一批数据，因为是手写图片数据，是个bx28x28的矩阵，我们直接
        # 把他拉伸成 bx784的向量，
        x = x.view(-1, 28 * 28)
        # 输入第一层，输出是bx1024
        x = self.fc1(x)
        ...
        # 激活一下
        x = F.relu(x)
        # 输入第三层，输入结果为bx10，为啥是10，因为标签是个长为10的one-hot编码
        return self.fc3(x)
...
if __name__ == '__main__':
    # 后续代码
    # 我们下载的是cpu版本的，只能cpu了，可以下载GPU的Pytorch版本，在GPU中运算会更快
    # my_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    my_device = torch.device('cpu')
    train_dataset = torchvision.datasets.MNIST(root='.', train=True, download=True, transform=tfs.ToTensor())
    test_dataset = torchvision.datasets.MNIST(root='.', train=False, download=True, transform=tfs.ToTensor())
