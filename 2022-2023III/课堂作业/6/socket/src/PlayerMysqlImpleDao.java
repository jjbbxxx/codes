...
public class PlayerMysqlImpleDao implements PlayerDao {
    ...
    public PlayerEntity getPlayer(String id) throws SQLException, ClassNotFoundException {
        String sql="select player_name from player where player_id="+id;
        System.out.println(sql);
        Connection conn = PlayerMysqlImpleDao.getConnection();
        //创建Statement对象
        PreparedStatement ps=conn.prepareStatement(sql);
        ResultSet rs = ((Statement) ps).executeQuery(sql);
        List<Map<String, Object>> list = rsToList(rs);
        closeConnection(conn, ps, rs);//关闭
        return (PlayerEntity) list;
    }
    @Override
    public int addPlayer(String id, String name) throws SQLException, ClassNotFoundException {
        String sql="insert into player values("+id+","+name+")";
        System.out.println(sql);
        Connection conn = PlayerMysqlImpleDao.getConnection();//连接
        PreparedStatement ps=conn.prepareStatement(sql);
